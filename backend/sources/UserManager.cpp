...
User* UserManager::loadUser(const std::string&email) {
    ...
    try {
        result = DatabaseModule::getInstance()->executeQuery(
...
User* UserManager::verifyLogin(const std::string&email,
...
bool UserManager::registerUser(const std::string&email,
    ...
    if (!loadUser(email)) {
        try {
        DatabaseModule::getInstance()->executeQuery(
            "INSERT INTO `users`(`ID`, `EMAIL`, `NAME`, `PASSWORD`) " +
            std::string("VALUES (NULL,'") + email + "','"
            + name + "','" + password + "')");
        return true;
        } catch (const DatabaseError&err) { }
    }
...
bool UserManager::saveUserData(const UserData&data) {
    try {
        if (loadUserData(data.getId())) {
            DatabaseModule::getInstance()->executeQuery(
                "UPDATE `users_data` SET ADDRESS='" + data.getAddress()
            ...
            DatabaseModule::getInstance()->executeQuery(
                "INSERT INTO `users_data`(`ID`, `ADDRESS`, `DOCUMENT`, `PHONE_NUMBER`) VALUES ('"
...
bool UserManager::removeUser(int id) {
    try {
        ...
        DatabaseModule::getInstance()->executeQuery(
            "DELETE FROM `users_data` WHERE `ID` = '" + _id + "'");
        return true;
    } catch (const DatabaseError&err) { }
